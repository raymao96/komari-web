...
export function ActionsCell({ row }: { row: Row<z.infer<typeof schema>> }) {
  ...
  const generateCommand = () => {
    ...
    switch (selectedPlatform) {
      case "linux":
        finalCommand =
          `wget -qO- https://gitlab.com/raymao96/komari-agent/-/raw/main/install.sh?ref_type=heads | sudo bash -s -- ` +
          quoteShellArgs(args);
        break;
      case "windows":
        finalCommand =
          `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ` +
          `"iwr 'https://gitlab.com/raymao96/komari-agent/-/raw/main/install.ps1?ref_type=heads'` +
      ...
      case "macos":
        finalCommand =
          `zsh <(curl -sL https://gitlab.com/raymao96/komari-agent/-/raw/main/install.sh?ref_type=heads) ` +
          quoteShellArgs(args);
        break;
    }
    return finalCommand;
